...
function BookingPreference(props) {
  ...
  let handleOpenModal = () => {
    if (timeSlot != "") $(".modalContainer").css("display", "block");
  };
  ...
  useEffect(() => {
    fetch(
      "https://elclasico-test.herokuapp.com/slots?venueId=" +
        venue_id +
        "&weekDayCode=*"
    )
      .then((res) => res.json())
    ...
    setDates([]);
    if (
      (nowMonth <= 7 && nowMonth % 2 != 0) ||
      (nowMonth >= 8 && nowMonth % 2 == 0)
    )
      lastDay = 31;
    else lastDay = 30;
    let flag = 0;
  ...
  const confirmBooking = async function () {
    ...
    await fetch("https://elclasico-test.herokuapp.com/bookings/", {
      ...
      .then((res) => res.json())
      .then((res) => {
        sessionStorage["bookingData"] = JSON.stringify({
          locality: locality,
          date: date,
          month: mlist[month - 1],
          slot: timeSlot,
        ...
        <div id="timeSlots">
          {slots[count].slots.map((s) => (
            <div
              ...
              data-price={s.cost}
            >
              {s.time} - {parseInt(s.time) + 1 < 10 ? "0" : ""}
              {parseInt(s.time) + 1}:00
            </div>
          ))}
        </div>
        <div className="priceDiv">
          ...
          <div className="row">
            <div className="col col_left">
              <p className="fadedText">Slot Cost</p>
            </div>
            <div className="col col_right">
              <p className="fadedText">Rs.{price}</p>
            </div>
          </div>
          <div className="row">
            <div className="col col_left">
              <p className="fadedText">Discount</p>
            </div>
            <div className="col col_right">
              <p className="fadedText">Rs.{discount}</p>
            </div>
          </div>
          <hr />
          <div className="row">
            <div className="col col_left">
              <p className="fadedText">Total</p>
            </div>
            <div className="col col_right">
              <p className="fadedText">Rs.{payable}</p>
            </div>
          </div>
        </div>
        <button
